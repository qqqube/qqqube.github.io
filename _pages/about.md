---
permalink: /
title: About
excerpt: "About me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

Hello, my name is Lauren! I am currently a visiting researcher in the Algorithmic Learning, Privacy, and Security (ALPS) lab at Stony Brook University, advised by [Ting Wang](https://alps-lab.github.io/). I am also working with researchers at Tsinghua University (THUNLP) under the supervision of [Zhiyuan Liu](https://nlp.csai.tsinghua.edu.cn/~lzy/).


I am a recent graduate from Columbia University, where I had the opportunity to work with [Junfeng Yang](https://www.cs.columbia.edu/~junfeng/) and [Asaf Cidon](https://www.asafcidon.com/). Before that, I received my Bachelor's degrees in Computer Science and Applied Mathematics from UC Berkeley.


## News 📅 

**[September 2023]** [Fewer is More: Trojan Attacks on Parameter-Efficient Fine-Tuning](https://arxiv.org/pdf/2310.00648.pdf) is now on arXiv!

**[September 2023]** [ToolLLM: Facilitating Large Language Models to Master 16000+ Real-World APIs](https://arxiv.org/pdf/2307.16789.pdf?trk=public_post_comment-text) is now on arXiv!

**[May 2023]** [Nazar: Monitoring and Adapting ML Models on Mobile Devices](https://arxiv.org/pdf/2305.07772.pdf) is now on arXiv!

